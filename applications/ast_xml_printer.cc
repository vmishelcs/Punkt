...
int main(int argc, char **argv) {
    if (argc != 3) {
		std::cerr << "AST XML printer application requires input and output file arguments"
                << std::endl;
		return 1;
	}
    ...
    std::ofstream output_file_stream(output_file_path, std::ios_base::trunc | std::ios_base::out);
