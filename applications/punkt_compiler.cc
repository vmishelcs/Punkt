...
static void MapOptions(int argc, char *argv[], std::map<int, std::string>& option_map);
...
int main(int argc, char *argv[]) {
	...
	if (dump_ast) {
		std::cout << "dumping AST to "
				<< (dump_ast_file_path.empty() ? "stdout" : dump_ast_file_path.string())
				<< std::endl;
	}
	if (dump_decorated_ast) {
		std::cout << "dumping decorated AST to "
				<< (dump_decorated_ast_file_path.empty() ? "stdout" : dump_decorated_ast_file_path.string())
				<< std::endl;
	}
