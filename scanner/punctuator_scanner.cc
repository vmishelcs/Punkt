...
std::unique_ptr<PunctuatorToken> PunctuatorScanner::ScanPunctuator() {
  std::string scanned_string = scanned.ToString();
  int num_punctuators_with_prefix =
      Punctuator::PunctuatorsWithPrefix(scanned_string);

  if (num_punctuators_with_prefix == 1 &&
      Punctuator::IsPunctuator(scanned_string)) {
    Punctuator punctuator(scanned_string);
    return std::make_unique<PunctuatorToken>(
        scanned_string, scanned.GetLocation(), std::move(punctuator));
  }

  if (num_punctuators_with_prefix == 0 || !input_stream->HasNext()) {
    FindLatestValidPunctuator();
    scanned_string = scanned.ToString();
    Punctuator punctuator(scanned_string);
    return std::make_unique<PunctuatorToken>(
        scanned_string, scanned.GetLocation(), std::move(punctuator));
  }
...
void PunctuatorScanner::FindLatestValidPunctuator() {
  while (!Punctuator::IsPunctuator(scanned.ToString()) && !scanned.IsEmpty()) {
