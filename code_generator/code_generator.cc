...
void CodeGenerator::WriteIR(std::unique_ptr<ParseNode> decorated_ast, fs::path output_file_path)
{
    CodeGenerator code_generator(std::move(decorated_ast), output_file_path);
    code_generator.WriteIRToOutputFile();
}

CodeGenerator::CodeGenerator(std::unique_ptr<ParseNode> decorated_ast, fs::path output_file_path) 
