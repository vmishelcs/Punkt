...
Scope::Scope(ScopeType scope_type, Scope *base_scope)
    : scope_type(scope_type), base_scope(base_scope), symbol_table() {}
...
SymbolTableEntry *Scope::Declare(const std::string &symbol,
                                 const TextLocation &tl, bool is_mutable,
                                 Type *type, SymbolType symbol_type) {
  return symbol_table.Insert(symbol, tl, is_mutable, type, symbol_type);
}
...
Scope *Scope::GetBaseScope() const { return base_scope; }

SymbolTableEntry *Scope::GetSymbolTableEntry(const std::string &symbol) {
  return symbol_table.Get(symbol);
}

ScopeType Scope::GetScopeType() const { return scope_type; }
