...
/// @brief This class is used to denote Punkt lambda types. Functions and lambda variables are given
/// the lambda type, consisting of a list of parameter types, and a return type. For example, a 
/// lambda type of `<integer, bool> -> integer` describes a lambda that takes two parameters (one
/// integer value and one boolean value), and returns an integer value.
class LambdaType : public Type {
public:
    /// @brief Create a `unique_ptr<LambdaType>` object specified by given parameter types and
    /// return type.
    /// @param parameter_types A `vector` of `Type` pointers describing the parameters of the
    /// `LambdaType`.
    /// @param return_type A pointer to a `Type` object describing the return type of the
    /// `LambdaType`.
    ...
    /// @brief Creates a `unique_ptr` to a `Lambda` object that is semantically equivalent to
    /// `this`.
