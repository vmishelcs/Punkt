...
void SemanticAnalysisVisitor::VisitLeave(ReturnStatementNode& node) {
  ...
  // If the enclosing lambda is declared as returning void, the return statement
...
void SemanticAnalysisVisitor::Visit(IdentifierNode& node) {
  if (IsBeingDeclared(node) || IsParameterIdentifier(node)) {
    // If an identifier is being declared, its semantic analysis is handled by
    // either `VisitLeave(DeclarationStatementNode&)` or
